...
public:
    ...
    // ————— METHODS ————— //
    Entity();
    Entity(GLuint texture_id, float speed, glm::vec3 acceleration, float jump_power, const int walking[2][11], float animation_time,
    ...
    void face_left() { m_face_left = true; }
    void face_right() { m_face_left = false; }
	...
	void const set_animation_row(int row) { m_current_animation_row = row; m_animation_indices = m_walking[row]; }
    ...
    void set_walking(const int walking[2][11])
    {
        for (int i = 0; i < 2; ++i)
        {
            for (int j = 0; j < 11; ++j)
